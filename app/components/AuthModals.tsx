...
import { storeAuthIntent, trackFunnelEvent } from '../lib/analytics-client'
...
export default function AuthModals() {
  ...
  const handleGoogleSignIn = () => {
    const intent = signupIntent || sessionStorage.getItem('fliply_signup_intent')
    const surface = sessionStorage.getItem('fliply_plan_surface') || 'signup_modal'
    if (intent) {
      storeAuthIntent(intent, 'google', surface)
      trackFunnelEvent('auth_started_with_plan_intent', {
        intent,
        method: 'google',
        surface,
        logged_in: false,
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const surface = sessionStorage.getItem('fliply_plan_surface') || 'signup_modal'
    if (intent) {
      storeAuthIntent(intent, 'email', surface)
      trackFunnelEvent('auth_started_with_plan_intent', {
        intent,
        method: 'email',
        surface,
